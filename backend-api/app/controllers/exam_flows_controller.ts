...
export default class ExamFlowsController {
  ...
  async enroll({ params, request, response, auth }: HttpContext) {
    ...
    for (const schedule of exam.schedules) {
      ...
      const start = DateTime.fromISO(schedule.date, { zone: 'Asia/Jakarta' }).set({
        hour: hours,
        minute: minutes,
        second: 0,
        millisecond: 0,
    ...
    if (!isValidSchedule) {
      return response.forbidden({
        message:
          'Ujian tidak dapat diakses saat ini. Jadwal ujian belum dimulai atau sudah berakhir.',
      })
    }
  ...
  async getQuestions({ params, response }: HttpContext) {
    ...
    const finalSerialized = clonedSections.map((sectionData: any) => {
      ...
      if (!isListening) {
        sectionData.questions = this.seededShuffle(
      ...
      sectionData.questions = sectionData.questions.map((q: any, idx: number) => ({
        ...q,
        answers: this.seededShuffle([...(q.answers || [])], seed + sectionData.id + idx + 7).map(
          (a: any) => ({
            id: a.id,
            answer: a.answer,
  ...
  async submitAnswer({ params, request, response }: HttpContext) {
    ...
    if (!question) {
      return response.forbidden({
        message: 'Akses ditolak: Pertanyaan tidak valid untuk ujian ini.',
      })
    }
    ...
    try {
      ...
      await mail.send((message) => {
        message
          ...
          .subject('Sertifikat & Hasil Ujian Anda').html(`
  ...
  async getResult({ params, response }: HttpContext) {
    ...
    console.log(
      `[getResult] Enroll #${enroll.id} | Status: ${enroll.status} | Submissions: ${enroll.submissions?.length || 0}`
    )
  ...
  async block({ params, response }: HttpContext) {
    ...
    console.log(
      `[block] Enroll #${enroll.id} | Old Status: ${enroll.status} | Submissions: ${enroll.$extras.submissions_count}`
    )
  ...
  async unblock({ params, response }: HttpContext) {
    ...
    console.log(
      `[unblock] Enroll #${enroll.id} | Submissions preserved: ${enroll.$extras.submissions_count}`
    )
