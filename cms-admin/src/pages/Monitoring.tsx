...
export default function Monitoring() {
  ...
  const handleKick = async (enrollId: number) => {
    if (!window.confirm("Apakah Anda yakin ingin mengeluarkan peserta ini?"))
      return;
    try {
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-center">
                    Wajah & Lokasi
                  </th>
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-center">
                    Aksi
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  ...
                  <tr>
                    <td
                      colSpan={7}
                      className="px-8 py-24 text-center text-slate-300 font-bold uppercase tracking-widest text-xs"
                    >
                    ...
                    >
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-bold text-slate-900">
                              {user.user?.name || `User ID: ${user.userId}`}
                            </p>
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.min(100, ((user.meta?.submissions_count || 0) / 140) * 100)}%`,
                              }}
                            />
                          </div>
                      ...
                      <td className="px-8 py-5">
                        ...
                        {user.snapshots?.find(
                          ...
                          (() => {
                            const snapLoc = user.snapshots.find(
                              (s) => s.latitude && s.longitude,
                            );
                            return (
                              <a
                        ...
                        {user.snapshots?.some((s) => s.audioUrl) && (
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              const latestAudio = user.snapshots?.find(
                                (s) => s.audioUrl,
                              );
                              if (latestAudio?.audioUrl) {
                                const audio = new Audio(
                                  BACKEND_URL + latestAudio.audioUrl,
                                );
                                audio.play();
                              }
                            }}
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">
                  Verifikasi Wajah — {selectedUser.examCode}
                </p>
              </div>
              <button
                onClick={() => setSelectedUser(null)}
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  {!selectedUser.snapshots ||
                  selectedUser.snapshots.length === 0 ? (
