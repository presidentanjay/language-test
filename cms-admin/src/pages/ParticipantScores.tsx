...
export default function ParticipantScores() {
  ...
  const filteredScores = scores.filter((item) => {
    ...
    const matchCategory = categoryFilter
      ? item.exam?.category === categoryFilter
      : true;
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="text-sm font-bold text-slate-400 uppercase tracking-[0.2em]">
            Daftar nilai dan status ujian peserta
          </p>
        </div>
                  ...
                  filteredScores.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <div
                          className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-sm font-black ${getScoreColor(item.score)}`}
                        >
