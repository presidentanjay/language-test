      ...
      <div className="max-w-4xl mx-auto space-y-8">
        <section className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-200 bg-gray-50">
            ...
            <p className="text-sm text-gray-500 italic mt-1 font-serif font-light">
              Update your account detail profile.
            </p>
          </div>
        ...
        <section className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-200 bg-gray-50">
            ...
            <p className="text-sm text-gray-500">
              Configure certificate signatures and background template
              dynamically.
            </p>
          </div>
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <Input
                label="Director Name (e.g. Ida Zuraida, Hj., S.S., M.Pd.)"
                value={directorName}
                onChange={(e) => setDirectorName(e.target.value)}
              />
              <Input
                label="Director Title / NIP (e.g. Head of Lembaga Bahasa)"
                value={directorNip}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">
                  Background Template Image (Optional)
                </label>
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => setCertTemplate(e.target.files?.[0] || null)}
                  className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm"
                />
                <p className="text-xs text-gray-400 mt-1">
                  Upload a new blank certificate template.
                </p>
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">
                ...
                <input
                  ...
                  accept="image/png"
                  onChange={(e) =>
                    setCertSignature(e.target.files?.[0] || null)
                  }
                  className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm"
                />
                <p className="text-xs text-gray-400 mt-1">
                  Upload the director's signature (transparent background).
                </p>
              </div>
            </div>
            <div className="flex justify-end">
              <Button
                onClick={handleUpdateCertSettings}
                disabled={isSavingCert || !directorName || !directorNip}
              >
                {isSavingCert ? (
                  <Loader2 className="h-4 w-4 animate-spin mr-2 inline" />
                ) : null}
