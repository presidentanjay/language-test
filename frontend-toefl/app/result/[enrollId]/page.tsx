...
export default function TestResult() {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-white">
                <div className="flex flex-col items-center gap-6">
                    ...
                    <p className="text-slate-400 font-black uppercase tracking-[0.3em] text-[10px]">Analyzing Your Performance...</p>
                </div>
            </div>
        );
    }
    ...
    const scores = enroll.sectionalScores || { listening: 0, structure: 0, reading: 0, overall: 0 };
                ...
                >
                    ...
                    <div className="p-10 md:p-16 text-center border-b border-slate-50">
                        ...
                        <div className="inline-flex items-center gap-2 bg-emerald-50 text-emerald-600 px-4 py-1.5 rounded-full mb-4 border border-emerald-100">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">Assessment Validated</span>
                        </div>
                        ...
                        <p className="text-slate-400 font-medium text-lg max-w-md mx-auto leading-relaxed">
                            Selamat! Anda telah menyelesaikan sesi sertifikasi <span className="text-slate-900 font-bold">{enroll.examCode}</span>.
                        </p>
                    </div>
                    ...
                    <div className="p-10 md:p-16">
                        <div className="grid md:grid-cols-2 gap-12 items-center">
                                ...
                                <div className="relative">
                                    <svg className="h-56 w-56 transform -rotate-90">
                                        ...
                                        <motion.circle
                                            ...
                                            animate={{ strokeDashoffset: 628 - (628 * (scores.overall / (isEPT ? 677 : 100))) }}
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Overall Score</span>
                                        <span className="text-6xl font-black text-slate-900 tracking-tighter">{scores.overall}</span>
                                        {isEPT && <span className="text-xs font-bold text-blue-600 uppercase mt-1">EPT Scale</span>}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">Performance Breakdown</h3>
                                
                                {isEPT ? (
                                    <>
                                        <ScoreBar label="Listening Comprehension" score={scores.listening} max={68} icon={<Headphones className="h-4 w-4" />} color="bg-blue-600" />
                                        <ScoreBar label="Structure & Written" score={scores.structure} max={68} icon={<Type className="h-4 w-4" />} color="bg-indigo-600" />
                                        <ScoreBar label="Reading Comprehension" score={scores.reading} max={67} icon={<BookOpen className="h-4 w-4" />} color="bg-violet-600" />
                                    </>
                                ) : (
                                    <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                                        <div className="flex items-center gap-4 mb-4">
                                            ...
                                            <div>
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Correct</p>
                                                <p className="text-xl font-black text-slate-900">{scores.overall} Points</p>
                                            </div>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="flex items-center gap-1 pt-4">
                                    {[...Array(5)].map((_, i) => (
                                        <Star key={i} className={`h-5 w-5 ${i < Math.round((scores.overall / (isEPT ? 677 : 100)) * 5) ? 'text-amber-400 fill-amber-400' : 'text-slate-100'}`} />
                                    ))}
                                    <span className="text-xs font-black ml-2 text-slate-900 uppercase">Achievement Level</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid sm:grid-cols-2 gap-4 mt-16">
                            ...
                            <button
                                ...
                                onClick={() => alert("Downloading official certificate...")}
                            >
    ...
    return (
        <div className="space-y-2">
            <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                ...
                <div className="text-slate-900">{score} <span className="text-slate-300">/ {max}</span></div>
            </div>
