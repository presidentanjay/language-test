...
export default function PaymentPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch exam details
      const examRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/exams/${examId}`, { headers })
      const examData = await examRes.json()
      setExam(examData)

      // Fetch payment check
      if (examData.code) {
        const payRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/payments/check/${examData.code}`, { headers })
        ...
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
          ...
          {exam && (
            <div className="space-y-4 mb-8">
              <div>
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Ujian</p>
                <p className="text-lg font-bold text-slate-900">{exam.title}</p>
              </div>
              <div>
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Kode</p>
                <p className="text-md font-bold text-slate-900">{exam.code}</p>
              </div>
              <div>
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Harga</p>
          ...
          {paymentStatus?.paid ? (
            ...
            >
              {paying ? 'Memproses...' : (exam?.price ? 'Bayar Sekarang' : 'Gratis')}
            </button>
          )}
        </div>
      </div>
