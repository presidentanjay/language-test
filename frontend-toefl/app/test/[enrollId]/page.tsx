          ...
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-2 text-blue-600">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest">
                Audio Sedang Berlangsung...
              </span>
            </div>
            <span className="text-[10px] font-bold text-slate-400">
              Jangan Me-refresh Halaman
            </span>
          </div>
...
export default function TestEngine() {
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (!violationTriggered.current) {
        ...
        e.returnValue =
          "Anda sedang mengerjakan ujian. Jika meninggalkan halaman, ujian akan direset!";
      }
    };
  ...
  const syncOfflineAnswers = useCallback(async () => {
    ...
    try {
      ...
      const promises = queue.map((item) =>
        api.post(`/enrolls/${enrollId}/submit`, {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        (e.ctrlKey || e.metaKey) &&
        ["c", "v", "p", "s", "u", "i", "j"].includes(e.key.toLowerCase())
      ) {
        e.preventDefault();
      }
    };
        ...
        <div className="max-w-md w-full mx-4">
          <div className="bg-white rounded-3xl p-10 shadow-2xl shadow-red-100 border border-red-100 text-center">
            ...
            <p className="text-slate-500 text-sm leading-relaxed mb-2">
              Anda terdeteksi membuka tab lain atau keluar dari mode Layar
              Penuh.
            </p>
            <p className="text-red-600 font-bold text-sm mb-8">
              Ujian Anda dihentikan sementara. Silakan hubungi Pengawas untuk
              membuka akses Anda kembali.
            </p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-left">
                  ...
                  <ul className="text-xs text-red-600 space-y-1">
                    ...
                    <li>
                      • Pengawas dapat mendiskualifikasi Anda jika terbukti
                      curang
                    </li>
                  </ul>
                </div>
        ...
        <div className="bg-white rounded-[32px] p-10 max-w-lg w-full text-center shadow-2xl shadow-blue-900/20 border border-slate-100 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-500 mb-8 font-medium leading-relaxed">
              Ujian ini sangat ketat dan mewajibkan mode{" "}
              <strong className="text-slate-900">
                Layar Penuh (Fullscreen)
              </strong>{" "}
              untuk mencegah kecurangan. Waktu ujian akan terus berjalan!
            </p>
            <button
      ...
      <div className="min-h-screen bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-6 fixed inset-0 z-[100]">
        <div className="bg-white rounded-[32px] p-8 max-w-md w-full text-center shadow-2xl relative overflow-hidden">
          ...
          <p className="text-slate-500 mb-8 font-medium">
            Pastikan semua soal telah terjawab. Ujian yang telah diselesaikan
  ...
  const activeAudioSegment = currentSection.sectionAudios?.find(
    (seg) =>
      currentQuestionIndex + 1 >= seg.fromQuestion &&
      currentQuestionIndex + 1 <= seg.toQuestion,
  );
          ...
          {(isOffline || pendingSync > 0) && (
            <div className="flex items-center gap-2 bg-yellow-50 px-3 py-1.5 rounded-full border border-yellow-200">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-yellow-700">
                {isOffline
                  ? "Offline - Jawaban Disimpan"
                  : `Menyinkronkan ${pendingSync} Jawaban...`}
              </span>
            </div>
          )}
          <div className="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-full border border-slate-100">
            <Clock
              className={`h-4 w-4 ${timeLeft < 300 ? "text-red-500 animate-pulse" : "text-slate-400"}`}
            />
            <span
              className={`text-sm font-mono font-bold ${timeLeft < 300 ? "text-red-600" : "text-slate-700"}`}
            >
              {formatTime(timeLeft)}
            </span>
          </div>
                  ...
                  {section.questions.map((q, qIndex) => {
                    ...
                    return (
                      <button
                        ...
                        className={`h-10 w-10 text-[11px] font-black rounded-xl transition-all border-2 flex items-center justify-center ${
                          isCurrent
            ...
            {displayAudioUrl && (
              <div className="bg-gradient-to-r from-indigo-50 via-blue-50 to-cyan-50 rounded-2xl p-5 mb-6 border border-blue-100 shadow-sm">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <p className="text-xs font-black text-blue-800 uppercase tracking-wider">
                      {activeAudioSegment
                        ? `Listening Segment (Q${activeAudioSegment.fromQuestion}-Q${activeAudioSegment.toQuestion})`
                        : "Listening Audio"}
                    </p>
                ...
                <LockedAudioPlayer
                  ...
                  id={
                    activeAudioSegment
                      ? `seg_${activeAudioSegment.id}`
                      : `sec_${currentSection.id}`
                  }
            ...
            >
                ...
                <div className="mb-10 p-8 bg-slate-50 rounded-[32px] border border-slate-100 relative overflow-hidden">
                  ...
                  {currentQuestion.direction && (
                    <div className="flex gap-4">
                      ...
                      <p className="text-sm text-slate-500 font-medium leading-relaxed italic whitespace-pre-line">
                        {currentQuestion.direction}
                      </p>
                    </div>
                  )}
              ...
              <div className="grid gap-4 mt-auto">
                {currentQuestion.answers.map((answer, index) => {
                  ...
                  const isSelected =
                    submissions[currentQuestion.id] === answer.id;
                  return (
                    <button
                      key={answer.id}
                      onClick={() =>
                      ...
                      className={`flex items-center p-6 rounded-[24px] border-2 transition-all text-left group relative overflow-hidden ${
                        isSelected
                    ...
                    >
                      <span
                        className={`h-10 w-10 rounded-xl flex items-center justify-center font-black mr-6 transition-all ${
                          isSelected
                      ...
                      <span
                        className={`font-black text-lg ${isSelected ? "text-white" : "text-slate-600 group-hover:text-slate-900"}`}
                      >
                        {answer.answer}
                      </span>
                      {isSelected && (
                ...
                onClick={() => {
                  if (currentQuestionIndex > 0)
                    ...
                    setCurrentQuestionIndex(
                      sections[prevSIndex].questions.length - 1,
                    );
                  }
                }}
                disabled={
