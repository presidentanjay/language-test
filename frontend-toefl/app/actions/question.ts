...
export async function createSection(examId: string | number, formData: FormData) {
    try {
        ...
        if (!res.ok) {
            ...
            return { success: false, error: errorData.message || "Failed to create section" };
        }
...
export async function deleteSection(sectionId: number | string) {
    try {
        ...
        if (!res.ok) {
            const errorData = await res.json().catch(() => ({}));
            return { success: false, error: errorData.message || "Failed to delete section" };
        }
...
export async function createQuestion(sectionId: string | number, formData: FormData) {
    try {
        ...
        if (!res.ok) {
            const errorData = await res.json().catch(() => ({}));
            return { success: false, error: errorData.message || "Failed to create question" };
        }
...
export async function deleteQuestion(questionId: number | string) {
    try {
        ...
        if (!res.ok) {
            const errorData = await res.json().catch(() => ({}));
            return { success: false, error: errorData.message || "Failed to delete question" };
        }
