...
export default function ExamPrep() {
    ...
    try {
      ...
      alert(
        error.response?.data?.message ||
          "Failed to enroll in exam. Please try again.",
      );
  ...
  const checkSchedule = () => {
    if (!exam?.schedules || exam.schedules.length === 0)
      return { isValid: false, message: "Belum Ada Jadwal" };
          ...
          <div className="lg:col-span-7 space-y-10">
            ...
            >
              ...
              <p className="text-slate-500 text-lg leading-relaxed font-medium">
                {exam?.description ||
                  "Persiapkan diri Anda untuk ujian sertifikasi resmi. Pastikan lingkungan tenang dan koneksi internet stabil."}
              </p>
            </motion.div>
            ...
            >
              ...
              <div className="grid sm:grid-cols-2 gap-6">
                {[
                  {
                    text: "Jangan berpindah tab browser",
                    icon: <Layers className="h-4 w-4" />,
                  },
                  {
                    text: "Jangan minimize jendela ujian",
                    icon: <Zap className="h-4 w-4" />,
                  },
                  {
                    text: "Sistem timer berbasis server",
                    icon: <Clock className="h-4 w-4" />,
                  },
                  {
                    text: "Pelanggaran mereset jawaban",
                ...
                {sections.map((section, i) => (
                  ...
                  >
                    <div className="h-14 w-14 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500">
                      {section.section.toLowerCase().includes("listening") ? (
                        <Headphones />
                      ) : section.section
                          .toLowerCase()
                          .includes("structure") ? (
                ...
                <div className="p-6 bg-blue-50 rounded-3xl border border-blue-100 mb-10">
                  <div className="flex gap-4">
                    ...
                    <p className="text-xs text-blue-900/60 leading-relaxed font-bold">
                      Dengan menekan tombol di bawah, Anda setuju untuk
                      mengikuti tata tertib ujian dan memulai timer sesi Anda.
                    </p>
                  </div>
                </div>
