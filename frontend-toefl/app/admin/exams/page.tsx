...
export default function AdminExamsPage() {
    ...
    const openCreate = () => { setEditExam(null); setForm(defaultForm); setFormError(""); setModalOpen(true); };
    const openEdit = (exam: Exam) => { setEditExam(exam); setForm({ ...exam }); setFormError(""); setModalOpen(true); };
    const closeModal = () => { setModalOpen(false); setEditExam(null); };

    const handleSave = async () => {
        if (!form.title || !form.code || !form.category) { setFormError("Judul, kode, dan kategori wajib diisi."); return; }
        setSaving(true);
        setFormError("");
        try {
            if (editExam) {
                const res = await api.put(`/exams/${editExam.id}`, form);
                setExams(prev => prev.map(e => e.id === editExam.id ? res.data : e));
            } else {
    ...
    const toggleActivated = async (exam: Exam) => {
        ...
        try {
            ...
            setExams(prev => prev.map(e => e.id === exam.id ? { ...e, activated: newVal } : e));
        } catch { }
    };
            ...
            <div className="flex-1 min-w-0 flex flex-col">
                ...
                <header className="bg-white border-b border-slate-100 h-16 px-6 flex items-center justify-between sticky top-0 z-30">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-lg font-black text-slate-900">Kelola Ujian</h1>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest hidden sm:block">Manajemen Program Ujian</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                ...
                <main className="flex-1 p-6">
                    {exams.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-64 bg-white border border-slate-100 rounded-3xl">
                            ...
                            <button onClick={openCreate} className="flex items-center gap-2 bg-blue-600 text-white text-xs font-black px-5 py-3 rounded-xl hover:bg-blue-700 transition-all">
                        ...
                        <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm">
                            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-50">
                                <span className="text-sm font-black text-slate-700">{exams.length} ujian terdaftar</span>
                            </div>
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="font-black text-slate-900 truncate">{exam.title}</span>
                                                <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md hidden sm:block">{exam.code}</span>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${exam.status === "publish" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}>
                                                    {exam.status === "publish" ? "Published" : "Draft"}
                                                </span>
                                                {exam.first_date && (
                                                    <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1">
                                                        <Calendar className="h-3 w-3" />{exam.first_date}
                                                    </span>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        {/* Activated toggle */}
                                        <button onClick={() => toggleActivated(exam)} title={exam.activated === "yes" ? "Nonaktifkan" : "Aktifkan"}
                                            className="hidden sm:flex items-center gap-1.5 text-xs font-black transition-colors shrink-0">
                                            {exam.activated === "yes"
                                                ? <ToggleRight className="h-7 w-7 text-emerald-500" />
                                                : <ToggleLeft className="h-7 w-7 text-slate-300" />}
                                            <span className={`hidden md:block ${exam.activated === "yes" ? "text-emerald-600" : "text-slate-400"}`}>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
                            ...
                            <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100 sticky top-0 bg-white rounded-t-[32px] z-10">
                                <div>
                                    <h2 className="text-xl font-black text-slate-900">{editExam ? "Edit Ujian" : "Tambah Ujian Baru"}</h2>
                                    <p className="text-xs text-slate-400 font-medium mt-0.5">{editExam ? `ID: ${editExam.id}` : "Isi detail ujian di bawah"}</p>
                                </div>
                                <button onClick={closeModal} className="h-10 w-10 bg-slate-50 hover:bg-slate-100 rounded-xl flex items-center justify-center transition-colors">
                            ...
                            <div className="px-8 py-6 space-y-5">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input className={inputClass} placeholder="TOEIC Prediction Test 1" value={form.title || ""} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} />
                                    </div>
                                    <div>
                                        <label className={labelClass}>Kode Ujian *</label>
                                        <input className={inputClass} placeholder="EPT-2025-01" value={form.code || ""} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} />
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <select className={inputClass} value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value as "ept" | "toeic" }))}>
                                    ...
                                    <div>
                                        ...
                                        <select className={inputClass} value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value as "publish" | "progress" }))}>
                                    ...
                                    <div>
                                        ...
                                        <select className={inputClass} value={form.activated} onChange={e => setForm(f => ({ ...f, activated: e.target.value as "yes" | "no" }))}>
                                ...
                                {/* Dates */}
                                <div>
                                    <label className={labelClass}>Jadwal Tanggal (maks. 4 sesi)</label>
                                    <div className="grid grid-cols-2 gap-3">
                                        {["first_date", "second_date", "third_date", "fourth_date"].map((key, i) => (
                                            <div key={key} className="flex items-center gap-2">
                                                <span className="text-[10px] font-black text-slate-400 w-4 shrink-0">S{i + 1}</span>
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        {["first_time", "second_time", "third_time", "fourth_time"].map((key, i) => (
                                            <div key={key} className="flex items-center gap-2">
                                                <span className="text-[10px] font-black text-slate-400 w-4 shrink-0">S{i + 1}</span>
                                ...
                                {/* Conference link */}
                                <div>
                                    <label className={labelClass}>Link Konferensi (opsional)</label>
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center text-slate-400">
                                            <Link2 className="h-4 w-4" />
                                        </div>
                                        <input className={inputClass + " pl-10"} placeholder="https://meet.google.com/..."
                                            value={form.conference_link || ""}
                                            onChange={e => setForm(f => ({ ...f, conference_link: e.target.value }))} />
                                    </div>
                                </div>
                            ...
                            <div className="px-8 py-5 border-t border-slate-100 flex justify-end gap-3">
                                <button onClick={closeModal} className="px-6 py-3 text-sm font-black text-slate-500 hover:text-slate-700 bg-slate-50 hover:bg-slate-100 rounded-xl transition-all">Batal</button>
                ...
                {deleteTarget && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
                            ...
                            <h3 className="text-xl font-black text-slate-900 mb-2">Hapus Ujian?</h3>
                            <p className="text-slate-500 text-sm mb-2">Aksi ini tidak dapat dibatalkan.</p>
                            <p className="text-slate-900 font-black text-sm mb-8 bg-slate-50 px-4 py-3 rounded-xl">"{deleteTarget.title}"</p>
                            <div className="flex gap-3">
                                <button onClick={() => setDeleteTarget(null)} className="flex-1 py-3 text-sm font-black bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-xl transition-all">Batal</button>
