...
import {
    GraduationCap, FileText, Users, BarChart3, Settings, Home,
    LogOut, Menu, User, Mail, Lock, CheckCircle2, AlertCircle,
    Eye, EyeOff, Shield, Globe,
} from "lucide-react";

interface Me { id: number; name: string; email: string; role: string; }
...
export default function AdminSettingsPage() {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                ...
                setMe(meRes.data);
                setForm(f => ({ ...f, name: meRes.data.name, email: meRes.data.email }));
            } catch {
    ...
    const handleSave = async () => {
        if (!form.name || !form.email) { setMsg({ type: "error", text: "Nama dan email wajib diisi." }); return; }
        if (form.password && form.password.length < 8) { setMsg({ type: "error", text: "Password minimal 8 karakter." }); return; }
        if (form.password && form.password !== form.confirm) { setMsg({ type: "error", text: "Konfirmasi password tidak cocok." }); return; }
        ...
        try {
            ...
            setMe(prev => prev ? { ...prev, name: form.name, email: form.email } : prev);
            setForm(f => ({ ...f, password: "", confirm: "" }));
            setMsg({ type: "success", text: form.password ? "Profil dan password berhasil diperbarui." : "Profil berhasil diperbarui." });
        } catch (err: any) {
            setMsg({ type: "error", text: err.response?.data?.message || "Gagal menyimpan perubahan." });
        } finally {
            setSaving(false);
        }
    };
    ...
    const inputClass = "w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-sm font-medium text-slate-900 outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-50 transition-all placeholder:text-slate-400";
                ...
                <header className="bg-white border-b border-slate-100 h-16 px-6 flex items-center justify-between sticky top-0 z-30">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest hidden sm:block">Konfigurasi Akun Admin</p>
                        </div>
                    </div>
                        ...
                        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                            ...
                            <div className="px-8 py-5 border-b border-slate-50 flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-slate-400 font-bold">Perbarui informasi akun dan password Anda</p>
                                </div>
                            </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        <div className="h-5 px-2 bg-amber-50 border border-amber-100 rounded-md flex items-center gap-1">
                                            ...
                                            <span className="text-[9px] font-black text-amber-600 uppercase tracking-wider">Admin</span>
                                        </div>
                                        <span className="text-xs text-slate-400 font-medium">{me?.email}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="px-8 py-6 space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-5 flex items-center text-slate-400"><User className="h-4 w-4" /></div>
                                        <input className={inputClass + " pl-12"} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-5 flex items-center text-slate-400"><Mail className="h-4 w-4" /></div>
                                        <input type="email" className={inputClass + " pl-12"} value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} />
                                    </div>
                                </div>
                                ...
                                <p className="text-[10px] text-slate-400 font-medium -mt-2">Kosongkan jika tidak ingin mengganti password.</p>

                                {/* New password */}
                                <div>
                                    <label className={labelClass}>Password Baru</label>
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-5 flex items-center text-slate-400"><Lock className="h-4 w-4" /></div>
                                        <input type={showPw ? "text" : "password"} className={inputClass + " pl-12 pr-12"}
                                    ...
                                    {form.password.length > 0 && (
                                        <div className="flex items-center gap-2 mt-2">
                                            <div className={`h-1 flex-1 rounded-full transition-colors ${form.password.length >= 8 ? "bg-emerald-400" : "bg-slate-100"}`} />
                                            <div className={`h-1 flex-1 rounded-full transition-colors ${form.password.length >= 12 ? "bg-emerald-400" : "bg-slate-100"}`} />
                                            <div className={`h-1 flex-1 rounded-full transition-colors ${/[^a-zA-Z0-9]/.test(form.password) ? "bg-emerald-400" : "bg-slate-100"}`} />
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                                {form.password.length < 8 ? "Lemah" : form.password.length < 12 ? "Sedang" : "Kuat"}
                                            </span>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-5 flex items-center text-slate-400"><Lock className="h-4 w-4" /></div>
                                        <input type={showConfirm ? "text" : "password"} className={inputClass + " pl-12 pr-12"}
                                            placeholder="Ulangi password baru" value={form.confirm}
                                    ...
                                    {form.confirm.length > 0 && (
                                        <p className={`text-[10px] font-black mt-2 ${form.password === form.confirm ? "text-emerald-600" : "text-red-500"}`}>
                                ...
                                {/* Unified msg */}
                                {msg && (
                                    <div className={`flex items-center gap-3 p-4 rounded-2xl text-sm font-bold ${msg.type === "success" ? "bg-emerald-50 text-emerald-700 border border-emerald-100" : "bg-red-50 text-red-600 border border-red-100"}`}>
