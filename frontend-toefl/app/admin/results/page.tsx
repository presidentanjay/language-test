...
export default function AdminResultsPage() {
  ...
  const passRate =
    dashStats && dashStats.stats.total_completed > 0
      ? Math.round(
          (dashStats.stats.total_completed /
            Math.max(dashStats.stats.total_participants, 1)) *
            100,
        )
      : 0;
      ...
      <div className="flex-1 min-w-0 flex flex-col">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest hidden sm:block">
                Statistik dan Monitoring Ujian
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
        ...
        <main className="flex-1 p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              {
                val: dashStats?.stats.total_participants ?? 0,
                label: "Total Peserta",
                sub: "Terdaftar",
              ...
              {
                val: dashStats?.stats.total_exams ?? 0,
                label: "Total Ujian",
                sub: "Program aktif",
              ...
              {
                val: dashStats?.stats.total_completed ?? 0,
                label: "Ujian Selesai",
                sub: "Berhasil finish",
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm">
              <div className="px-6 py-4 border-b border-slate-50 flex items-center gap-3">
                ...
                <span className="text-[10px] font-black text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full ml-auto">
                  {monitoring.length} aktif
                </span>
              </div>
              {monitoring.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-slate-400">
                  ...
                  <p className="text-xs text-slate-300 mt-1">
                    Peserta yang sedang ujian akan muncul di sini
                  </p>
                </div>
              ) : (
                    ...
                    >
                      <div>
                        <div className="font-black text-slate-900 text-sm">
                          ID Peserta #{enroll.user_id}
                        </div>
                        <div className="flex items-center gap-2 mt-1">
                          <span
                            className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${enroll.for === "ept" ? "bg-blue-50 text-blue-600" : "bg-indigo-50 text-indigo-600"}`}
                          >
                            {enroll.for?.toUpperCase()}
                          </span>
                          <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1">
                            <BookOpen className="h-3 w-3" />{" "}
                            {enroll.submissions?.length ?? 0} jawaban
                          </span>
                        </div>
                      </div>
              ...
              dashStats.recentActivity.length === 0 ? (
                ...
                <div className="divide-y divide-slate-50">
                  {dashStats.recentActivity.map((enroll, i) => {
                    const badge =
                      statusBadge[enroll.status] ?? statusBadge.enrolled;
                    return (
                      <motion.div
                        key={enroll.id}
                        initial={{ opacity: 0 }}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          />
                          <span
                            className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-full ${badge.class}`}
                          >
                            {badge.label}
                          </span>
                        </div>
          ...
          >
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-8">
              <div className="flex-1">
                <p className="text-blue-200 text-sm font-black uppercase tracking-widest mb-2">
                  Ringkasan Platform
                </p>
                ...
                <p className="text-blue-200 text-sm">
                  dari {dashStats?.stats.total_participants ?? 0} peserta
                  terdaftar
                </p>
              </div>
              <div className="flex gap-10">
                {[
                  ...
                  <div key={i} className="text-center">
                    ...
                    <div className="text-[10px] font-black text-blue-200 uppercase tracking-widest mt-1">
                      {s.label}
                    </div>
                  </div>
                ))}
