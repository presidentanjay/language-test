      ...
      <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
        ...
        >
          ...
          <div className="flex flex-col">
            <span
              className={`font-black text-lg leading-tight tracking-tight uppercase ${isDark ? "text-white" : "text-slate-900"}`}
            >
              Lembaga Bahasa
            </span>
        ...
        {/* Desktop Links */}
        <div
          className={`hidden md:flex items-center gap-8 text-xs font-bold uppercase tracking-widest ${isDark ? "text-slate-400" : "text-slate-500"}`}
        >
          {navLinks.map((link) => (
            <button
              key={link.name}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className={`absolute top-full left-0 w-full border-t flex flex-col p-6 gap-6 md:hidden shadow-2xl ${
              isDark
