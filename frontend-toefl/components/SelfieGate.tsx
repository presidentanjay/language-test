...
export default function SelfieGate({ enrollId, onVerified }: SelfieGateProps) {
    ...
    const startCamera = async () => {
      try {
        const stream = await navigator.mediaDevices.getUserMedia({
          ...
          audio: true, // Request microphone access at the beginning
        });
        if (cancelled) {
        ...
        if (!cancelled) {
          if (err.name === "NotAllowedError") {
            setCameraError(
              "Izin kamera ditolak. Harap izinkan akses kamera untuk melanjutkan ujian.",
            );
        ...
        if (blob) {
          ...
          if ("FaceDetector" in window) {
            try {
              ...
              if (faces.length > 1) {
                setError(
                  "Terdeteksi lebih dari satu wajah! Pastikan hanya ada Anda di dalam bingkai.",
                );
                return;
              }
            } catch (err) {
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          (err) => {
            if (err.code === err.PERMISSION_DENIED) {
              reject(
                new Error(
                  "Akses lokasi ditolak. Anda wajib mengizinkan akses lokasi (GPS) untuk ujian ini.",
                ),
              );
            } else {
              reject(
                new Error("Gagal mendapatkan lokasi. Pastikan GPS Anda aktif."),
              );
            }
          },
          {
  ...
  const confirmAndUpload = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err.message || "Gagal mengunggah foto atau lokasi. Silakan coba lagi.",
      );
    } finally {
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="bg-red-50 rounded-xl p-4 border border-red-100">
            ...
            <ul className="text-[11px] font-medium text-red-700 space-y-2">
              <li>
                1. Dilarang keras menekan tombol <b>Kembali / Back</b> di
                peramban.
              </li>
              <li>
                2. Membuka tab baru atau aplikasi lain akan memicu sistem{" "}
                <b>Peringatan Anti-Curang</b> dan ujian dapat dihentikan.
              </li>
              <li>
                3. Sistem AI kamera akan memantau jumlah orang di layar secara
                diam-diam.
              </li>
              <li>
                4. Koneksi internet terputus tidak akan menghilangkan jawaban
                yang sudah disimpan.
              </li>
            </ul>
          </div>
